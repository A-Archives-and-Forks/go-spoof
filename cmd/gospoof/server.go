...
func (s *server) handleConnection(conn net.Conn, config Config) {
	...
	var err error
	// Important: never write a banner before SSH handshake
	if originalPort != 22 {
...
func (v *vFileSystem) exists(p string) bool               { p = path.Clean(p); if v.isDir(p) { return true }; _, ok := v.files[p]; return ok }
func (v *vFileSystem) readFile(p string) (string, bool)   { c, ok := v.files[path.Clean(p)]; return c, ok }
func (v *vFileSystem) writeFile(p, content string, ap bool) { p = path.Clean(p); v.ensureParent(p); if ap { v.files[p] = v.files[p] + content; return }; v.files[p] = content }
func (v *vFileSystem) listDir(p string) []string {
	...
	for k := range children { out = append(out, k) }
	sort.Strings(out)
	return out
}
...
func resolvePath(cwd, p string) string {
	if p == "" { return cwd }
	if strings.HasPrefix(p, "/") { return path.Clean(p) }
...
func appendKeylog(ip string, port uint16, session string, data string) {
	...
	if err != nil { log.Println("keylog write failed:", err); return }
	defer f.Close()
...
func runHoneypotSession(conn net.Conn, originalPort uint16) {
	...
	for {
		bw.WriteString("login: "); bw.Flush()
		u, _ := br.ReadString('\n'); u = strings.TrimSpace(u)
		bw.WriteString("Password: "); bw.Flush()
		p, _ := br.ReadString('\n'); p = strings.TrimSpace(p)
		appendKeylog(remoteIP, originalPort, sessionID, "login_attempt user="+u+" pass="+p)
		if u == "root" && p == "root" { appendKeylog(remoteIP, originalPort, sessionID, "login_success user=root"); break }
		bw.WriteString("Login incorrect\n"); bw.Flush()
	}
	user := "root"

	vfs := newVFS()
	if motd, ok := vfs.readFile("/etc/motd"); ok { bw.WriteString(motd) }
	...
	for {
		...
		if cmdline == "" { writePrompt(); continue }
		parts := strings.Fields(cmdline); cmd := parts[0]; args := parts[1:]
		switch cmd {
		case "exit","quit","logout": bw.WriteString("logout\n"); bw.Flush(); return
		case "help": bw.WriteString("Commands: help, whoami, pwd, ls, cd, cat, echo, touch, mkdir, uname, id, ifconfig, ps, clear, exit\n")
		case "whoami": bw.WriteString(user + "\n")
		case "pwd": bw.WriteString(cwd + "\n")
		case "uname": bw.WriteString("Linux\n")
		case "id": bw.WriteString("uid=0(root) gid=0(root) groups=0(root)\n")
		case "ifconfig","ip": bw.WriteString("eth0: inet 10.0.0.5  netmask 255.255.255.0  broadcast 10.0.0.255\n")
		case "ps": bw.WriteString("  PID TTY          TIME CMD\n    1 ?        00:00:00 init\n  742 ?        00:00:00 sshd\n")
		case "clear": bw.WriteString("\033[H\033[2J")
		case "ls":
			target := cwd; if len(args)>0 { target = resolvePath(cwd, args[0]) }
			if vfs.exists(target) && vfs.isDir(target) {
				items := vfs.listDir(target); if len(items)==0 { bw.WriteString("\n") } else { bw.WriteString(strings.Join(items, "  ")+"\n") }
			} else if vfs.exists(target) { bw.WriteString(path.Base(target)+"\n")
			} else { bw.WriteString("ls: cannot access '" + target + "': No such file or directory\n") }
		case "cd":
			dest := "/root"; if len(args)>0 { dest = resolvePath(cwd, args[0]) }
			if vfs.exists(dest) && vfs.isDir(dest) { cwd = dest
			} else if vfs.exists(dest) { bw.WriteString("bash: cd: "+dest+": Not a directory\n")
			} else { bw.WriteString("bash: cd: "+dest+": No such file or directory\n") }
		case "cat":
			if len(args)==0 { bw.WriteString("cat: missing file operand\n"); break }
			for _, a := range args {
				fp := resolvePath(cwd, a)
				if vfs.isDir(fp) { bw.WriteString("cat: "+a+": Is a directory\n"); continue }
				if content, ok := vfs.readFile(fp); ok { bw.WriteString(content) } else { bw.WriteString("cat: "+a+": No such file or directory\n") }
			}
		case "echo":
			if len(args)==0 { bw.WriteString("\n"); break }
			joined := strings.Join(args," "); redir := ""; var target string
			if i := strings.Index(joined, ">>"); i>=0 { redir=">>"; target=strings.TrimSpace(joined[i+2:]); joined=strings.TrimSpace(joined[:i])
			} else if i := strings.Index(joined, ">"); i>=0 { redir=">"; target=strings.TrimSpace(joined[i+1:]); joined=strings.TrimSpace(joined[:i]) }
			if redir=="" { bw.WriteString(joined+"\n") } else { vfs.writeFile(resolvePath(cwd,target), joined+"\n", redir==">>") }
		case "touch":
			if len(args)==0 { bw.WriteString("touch: missing file operand\n"); break }
			for _, a := range args { vfs.writeFile(resolvePath(cwd,a), "", true) }
		case "mkdir":
			if len(args)==0 { bw.WriteString("mkdir: missing operand\n"); break }
			for _, a := range args { vfs.ensureDir(resolvePath(cwd,a)) }
		default: bw.WriteString(cmd + ": command not found\n")
		}
		bw.Flush(); writePrompt()
	}
}
...
func serveSSH(conn net.Conn, originalPort uint16) error {
	...
	cfg := &ssh.ServerConfig{
		PasswordCallback: func(c ssh.ConnMetadata, pass []byte) (*ssh.Permissions, error) {
			if c.User() == "root" && string(pass) == "root" { return nil, nil }
	...
	for chReq := range chans {
		if chReq.ChannelType() != "session" { _ = chReq.Reject(ssh.UnknownChannelType, "unknown"); continue }
...
// PTY-aware line reader with echo/backspace handling
func readLinePTY(br *bufio.Reader, bw *bufio.Writer) (string, error) {
	...
	for {
		...
		switch ch {
		case '\r', '\n':
			// eat optional paired byte and echo CRLF
			if ch == '\r' {
		...
		case 0x1b: // escape seq (arrows etc.): swallow a few bytes
			// typical CSI sequences are 3 bytes: ESC [ X
...
func runHoneypotSessionSSH(rw io.ReadWriter, remoteIP string, originalPort uint16, _ bool) {
	...
	for {
		...
		case "exit","quit","logout": bw.WriteString("logout\n"); bw.Flush(); return
		case "help": bw.WriteString("Commands: help, whoami, pwd, ls, cd, cat, echo, touch, mkdir, uname, id, ifconfig, ps, clear, exit\n")
		case "whoami": bw.WriteString("root\n")
		case "pwd": bw.WriteString(cwd + "\n")
		case "uname": bw.WriteString("Linux\n")
		case "id": bw.WriteString("uid=0(root) gid=0(root) groups=0(root)\n")
		case "ifconfig","ip": bw.WriteString("eth0: inet 10.0.0.5  netmask 255.255.255.0  broadcast 10.0.0.255\n")
